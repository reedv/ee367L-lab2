...
int main(void)
{
	int sock_filedes, new_filedes;  // listen on sock_filedes, new connection on new_filedes
	struct addrinfo hints, *servinfo, *p;  // addrinfo structs
	...
	for(p = servinfo; p != NULL; p = p->ai_next) {
		if ((sock_filedes = socket(p->ai_family, p->ai_socktype,
				p->ai_protocol)) == -1) {
			perror("server: socket");
			continue;
		}

		if (setsockopt(sock_filedes, SOL_SOCKET, SO_REUSEADDR, &yes,
				sizeof(int)) == -1) {
			perror("setsockopt");
			exit(1);
		}

		if (bind(sock_filedes, p->ai_addr, p->ai_addrlen) == -1) {
			close(sock_filedes);
			perror("server: bind");
			continue;
		}
	...
	while(1) {  // main accept() loop
		...
		if (!fork()) { // this is the child process
			close(sock_filedes); // child doesn't need the listener
